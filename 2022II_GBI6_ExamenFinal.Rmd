...
-   En la primera grafica se puede ver que el eje de las x son las islas donde se encuentra cada especie, el eje de las y son las especies de pinguinos, tambien se debe de tener en cuenta que los pinguinos fueron agrupados de acuerdo al sexo que tienen

-   En la segunda grafica se hace un diagrama de dispercion donde se tiene en cuenta las alturas de los picos, siendo el eje de las x las especies de los pinguinos, tambien se debe tener en cuenta que la especie Chinstrap y Gentoo tienen los picos mas largos

-   En el tercer grafico se puede ver una relacion entre la especie y el ancho de los picos, siendo la especie con mas ancho de pico el Chinstrap
...
```{r}
p4=  pinguinos %>% ggplot(aes(x= body_mass_g, y =flipper_length_mm, color = factor(sex)))+  geom_point(size=0.5, alpha=0.5) + geom_smooth(method = "lm", se = FALSE, na.rm = TRUE) +
  facet_grid(factor(species)~factor(island)) + theme_classic() + theme(legend.position = "none") +
  stat_poly_eq(coef.digits = 2,use_label(c("eq", "adj.R2")), formula = y ~ poly(x,1, raw = TRUE), 
               label.x.npc = 0, label.y.npc = 1.0) +
...
Se puede ver que existe variaciones en cada una de las especies y dependiendo de las islas donde se encuentran el pico y la masa corporal cambia
...
-   En el grafico se puede observar que en el grafico a medidad que ocupan el suero humano en un tiempo de 24 horas la distancia de los genes no es mucha, en este caso se puede observar que en las columnas x2, x16 y x9 no tienen una distancia que sobrepasa de un valor de 100, por otra parte se puede observar que al tratar con el suero inactivado por calor y con un tiempo de 48 hrs la distancia sobrepasa de 100.
